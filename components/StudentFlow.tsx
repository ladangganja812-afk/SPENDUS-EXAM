...
export const StudentFlow: React.FC<StudentFlowProps> = ({ user, onStartExam, onLogout, settings, onRefreshSettings }) => {
  ...
  const loadExamsAndResults = async () => {
    ...
    const filteredExams = exams.filter(e => {
        ...
        // Validasi apabila mereka di mapping khusus dan memiliki custom date
        if (isMapped) {
            const specificMapping = updatedMappings.find((m: any) => m.examId === e.id);
            if (specificMapping?.examDate?.includes('|')) { // Tryout Mode date range
                const [start, end] = specificMapping.examDate.split('|');
                if (todayStr >= start && todayStr <= end) isValidForToday = true;
            } else if (specificMapping?.examDate) {
                if (specificMapping.examDate === todayStr) isValidForToday = true;
            } else {
                // If mapping exist but no custom date, fallback to exam's globally configured date
                if (e.examDate === todayStr) isValidForToday = true;
            }
        } 

        // Validasi apabila dia mendapatkan akses dari Sekolah (berlaku untuk semua murid di sekolah itu di hari itu)
        if (hasSchoolAccess && !isValidForToday) {
             if (e.examDate === todayStr) isValidForToday = true;
        }

        return isValidForToday;
    });
  ...
  if (step === 'DASHBOARD') {
    return (
      <div className="min-h-screen flex flex-col items-center pt-10 px-4 pb-10 overflow-hidden relative" style={themeStyle}>
        ...
        {/* Floating Footer */}
        {settings.footerText && (
            <div className="fixed bottom-6 w-full text-center z-20 pointer-events-none print:hidden">
